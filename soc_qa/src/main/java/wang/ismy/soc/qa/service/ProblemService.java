package wang.ismy.soc.qa.service;
...
import wang.ismy.soc.qa.dao.ProblemDao;
...
public class ProblemService {
    ...
    public PageResult<Problem> newList(String labelId, Integer page, Integer size) {
        page--;

        if (page<0){
        	page =0;
		}
		Page<Problem> problems = problemDao.newList(labelId, PageRequest.of(page, size));
        PageResult<Problem> result = new PageResult<>();
        result.setTotal(problems.getTotalElements());
        result.setData(problems.getContent());
        return result;
	}

	public PageResult<Problem> hotList(String labelId, Integer page, Integer size) {
		page--;
		if (page<0){
			page =0;
		}
		Page<Problem> problems = problemDao.hotList(labelId, PageRequest.of(page, size));
		PageResult<Problem> result = new PageResult<>();
		result.setTotal(problems.getTotalElements());
		result.setData(problems.getContent());
		return result;
	...
	public PageResult<Problem> waitList(String labelId, Integer page, Integer size) {
		page--;

		if (page<0){
			page =0;
		}
		Page<Problem> problems = problemDao.waitList(labelId, PageRequest.of(page, size));
		PageResult<Problem> result = new PageResult<>();
		result.setTotal(problems.getTotalElements());
		result.setData(problems.getContent());
		return result;
